...
#if defined(NDEBUG)
# error "Ecologicalsquare cannot be compiled without assertions."
#endif
...
CBigNum bnProofOfStakeLimitV2(~uint256(0) >> 16);

int nStakeMinConfirmations = 150;
unsigned int nStakeMinAge = 8 * 60 * 60; // 8 hours
unsigned int nModifierInterval = 1 * 60; // time to elapse before new modifier is computed

//pow Maturity confirmation
...
const string strMessageMagic = "Ecologicalsquare Signed Message:\n";
...
{
    SetNull();
    string hs = prevout.hash.ToString();
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Ecologicalsquare: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Ecologicalsquare: find block wanted by given orphan block
...
{
    int64_t nSubsidy = (nCoinAge  * 0.001 * COIN) / 365;
    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 16 * 60;  // 16 mins

// Ecologicalsquare: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Ecologicalsquare miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Ecologicalsquare: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Ecologicalsquare: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Ecologicalsquare: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Ecologicalsquare: track money supply and mint amount info
...
// Ecologicalsquare: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Ecologicalsquare: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Ecologicalsquare: compute stake entropy bit for stake modifier
    ...
    // Ecologicalsquare: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Ecologicalsquare: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Ecologicalsquare: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Ecologicalsquare: attempt to generate suitable proof-of-stake
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    // if we are trying to sign
    //    something except proof-of-stake block template
    if(!IsProofOfStake() && nFees == -1)
    {
...
{
    RenameThread("Ecologicalsquare-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        bool connectself = GetBoolArg("-connectself",false);
        if (connectself == false && nNonce == nLocalHostNonce && nNonce > 1)
        {

...
void Shutdown()
{
    ...
    RenameThread("Ecologicalsquare-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Ecologicalsquare.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Ecologicalsquared.pid)") + "\n";
    ...
    strUsage += "  -genproclimit=<n>     " + _("Pow Gen Thread (default: 2*cpunum)") + "\n";
#ifdef USE_UPNP
#if USE_UPNP
    ...
    strUsage += "  -connectself=<false>                 " + _("treat self as a node. default:false") + "\n";

    return strUsage;
}

/** Sanity checks
 *  Ensure that Ecologicalsquare is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Ecologicalsquare is shutting down."));
...
#endif
    // Make sure only a single Ecologicalsquare process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Ecologicalsquare is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Ecologicalsquare version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Ecologicalsquare server starting\n");
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // Ecologicalsquare: reserve balance amount
    {
    ...
    // requested to kill Ecologicalsquare during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Ecologicalsquare") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Ecologicalsquare to complete") << "\n";

...
#define USE_OLDHASH 0

/** The maximum allowed size for a serialized block, in bytes (network rule) */
//static const unsigned int IPO_VALUES  = 100000
...
static const int64_t IPO_SHARE = 630000000 * COIN;
static const int64_t COIN_YEAR_REWARD = 1 * CENT; // 1% per year

inline bool IsProtocolV1RetargetingFixed(int nHeight) { /*return TestNet() || nHeight > 38423;*/return true; }
inline bool IsProtocolV2(int nHeight) { /*return TestNet() || nHeight > 319000;*/ return true; }
inline bool IsProtocolV3(int64_t nTime) { return TestNet() || nTime > 2444028400; }

inline int64_t FutureDriftV1(int64_t nTime) { return nTime + 10 * 60; }
inline int64_t FutureDriftV2(int64_t nTime) { return nTime + 15; }
inline int64_t FutureDrift(int64_t nTime, int nHeight) { return IsProtocolV2(nHeight) ? FutureDriftV2(nTime) : FutureDriftV1(nTime); }

inline int64_t PastDriftV1(int64_t nTime) { return nTime - 10 * 60; }
inline int64_t PastDriftV2(int64_t nTime) { return nTime - 15; }
inline int64_t PastDrift(int64_t nTime, int nHeight)   { return  IsProtocolV2(nHeight) ? PastDriftV2(nTime) : PastDriftV1(nTime); }
...
void ThreadBitcoinMiner(CWallet *pwallet);
...
public:
    ...
    {
        // Ecologicalsquare: the coin stake transaction is marked with the first output empty
...
public:
    ...
    // Ecologicalsquare: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Ecologicalsquare: two types of block: proof-of-work or proof-of-stake
    ...
    // Ecologicalsquare: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Ecologicalsquare: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Ecologicalsquare: block index flags
    enum  
    {

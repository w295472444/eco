...
//////////////////////////////////////////////////////////////////////////////
//
// EcologicalsquareMiner
//
extern bool fRequestShutdown;
extern unsigned int nMinerSleep;

volatile bool fStopPos = true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey, bool fProofOfStake, int64_t* pFees)
{
    ...
    if (!fProofOfStake)
    {
        ...
        /*CScript& cs = txNew.vout[0].scriptPubKey;
        len = cs.size();
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("Ecologicalsquare-Stakeminer");
    ...
    while (true)
    {
        ...
        if(pindexBest->nHeight + 1 <=Params().LastPOWBlock())
        {
            MilliSleep(1000);
            continue;
        }
        ...
        {
            fTryToSync = false;
            if (vNodes.size() < 3 || pindexBest->GetBlockTime() < GetTime() - 60)
            {
                MilliSleep(1000);
                continue;
            }
        ...
        {
            //printf("Pos SignBlock failed nFees = %lld\n",nFees);

...
// Copyright (c) 2015 Ecologicalsquare Coin developers
...
{
    nStakeModifier = 0;
    string ha = hashBlockFrom.ToString();
    size_t size = mapBlockIndex.size();
    if (!mapBlockIndex.count(hashBlockFrom))
    {
        char  ch[1024] = {'\0'};
        sprintf(ch,"GetKernelStakeModifier() %s : block not indexed\n",ha.c_str());
        return error(ch);
    }
    ...
    nStakeModifierSelectionInterval = 60 ;
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
        ...
        {   // reached best block; may happen if node is behind on block chain
            ...
            {
                printf("GeneratedStakeModifier   failed\n");
                return false;
            }
        }
        pindex = pindex->pnext;
...
// Ecologicalsquare kernel protocol
// coinstake must meet hash target according to the protocol:
// kernel (input 0) must meet the formula
